...
export function TaskBoard({ projectId, projectRevision, hasCurrentCommitment }: TaskBoardProps) {
  ...
  function draft(task: Task) {
    return drafts[task.id] ?? { status: task.status, due: task.due ?? "", note: task.note ?? "" };
  }
  ...
  async function update(task: Task) {
    ...
    try {
      accept(await api.updateTask({ project_id: projectId, expected_revision: data.revision, id: task.id, status: value.status, due: value.due || null, note: value.note || null }));
    } catch {
      setMessage(t("task.conflict"));
      await client.invalidateQueries({ queryKey: key });
    }
