    ...
    function invoke(cmd: string, args?: { input?: any; options?: any }): Promise<unknown> {
      ...
      switch (cmd) {
        ...
        case "add_task": {
          ...
          list.push({ id: `task-${list.length + 1}`, text: input.text, status: "open", unclear: input.unclear, due: null, note: null, tags: [], commits: [], adopted_from_proposal_id: null, was_committed: false, is_current_commitment: false, updated_at: "2026-08-12T09:00:00Z" });
        ...
        case "adopt_subtasks": {
          const list = tasksByProject[input.project_id] ?? (tasksByProject[input.project_id] = []);
          for (const text of input.texts) list.push({ id: `task-${list.length + 1}`, text, status: "open", unclear: false, due: null, note: null, tags: [], commits: [], adopted_from_proposal_id: input.proposal_id, was_committed: false, is_current_commitment: false, updated_at: "2026-08-12T09:00:00Z" });
          taskRevision[input.project_id] = (taskRevision[input.project_id] ?? 1) + 1;
          return Promise.resolve({ revision: String(taskRevision[input.project_id]), tasks: list });
        }
